...
exports.ClassificationValidator = void 0;
class ClassificationValidator {
    static validateProduct(product) {
        ...
        for (const [ingredient, classification] of product.ingredientAnalysis.classification) {
            ...
            if (dualSourceIngredients.has(normalizedIngredient)) {
                if (normalizedClassification === 'natural' ||
            ...
            if (product.ingredientAnalysis.environmentalImpact) {
                const impactData = product.ingredientAnalysis.environmentalImpact.analysis.find(item => item.ingredient.toLowerCase() === normalizedIngredient);
                if (!impactData) {
                    issues.push({
                        ingredient,
                        type: 'environmental',
    ...
    static calculateClassificationConfidence(ingredient, classification) {
...
exports.ClassificationValidator = ClassificationValidator;
ClassificationValidator.KNOWN_NATURAL_INGREDIENTS = new Set([
    'water',
    'aloe',
    'coconut',
    'shea',
...
ClassificationValidator.KNOWN_SYNTHETIC_INGREDIENTS = new Set([
    'paraben',
    'sulfate',
    'silicone',
    'petrolatum',
